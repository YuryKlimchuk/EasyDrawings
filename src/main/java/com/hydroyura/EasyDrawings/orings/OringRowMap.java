...
public class OringRowMap implements RowMapper<Oring> {
	...
	public Oring mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if(Constants.isThere(rs, Constants.TABLE_COLOMN_ORINGS_INTERNAL_DIAMETER)) oring.setInternalDiameter(rs.getFloat(Constants.TABLE_COLOMN_ORINGS_INTERNAL_DIAMETER));
